...
export function getWebsiteStructuredData(url: string = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000') {
  return {
    ...
    "url": url,
    "description": "Banco de questões do ENEM totalmente gratuito e sem limite de uso. Estude sem pagar nada, com acesso ilimitado a todas as questões de provas anteriores.",
    "keywords": "ENEM gratuito, questões ENEM grátis, banco questões gratuito, vestibular sem pagar, ENEM sem limite",
    "potentialAction": {
      "@type": "SearchAction",
      "target": {
        "@type": "EntryPoint",
...
export function getEducationalAppStructuredData(url: string = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000') {
  return {
    ...
    "description": "Plataforma de estudo com questões de provas anteriores do ENEM totalmente GRATUITA e sem limite de uso. Estude sem pagar nada e sem restrições.",
    "url": url,
